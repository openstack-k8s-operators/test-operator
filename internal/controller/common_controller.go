...
type TestResourceConfig[T TestResource] struct {
	...
	GenerateServiceConfigMaps func(ctx context.Context, helper *helper.Helper, instance T, workflowStepIndex int) error
...
func CommonReconcile[T TestResource](
	...
	if config.NeedsConfigMaps {
		err = config.GenerateServiceConfigMaps(ctx, helper, instance, workflowStepIndex)
		if err != nil {
			conditions.Set(condition.FalseCondition(
