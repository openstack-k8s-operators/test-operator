...
Now that test-operator is part of the OpenStack product, running changes
locally may be interrupted by the :code:`test-operator-controller-manager`
pod, which runs by default. To prevent errors, disable test-operator
before testing local changes by following these steps.
