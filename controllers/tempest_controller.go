...
func buildTempestPod(
	...
	podName := GetPodName(instance, workflowStep)
...
func setTempestConfigVars(
	...
	envVars["TEMPEST_CONCURRENCY"] = GetDefaultInt(numValue)
	...
	for _, extraImageDict := range extraImages {
		...
		envVars["TEMPEST_EXTRA_IMAGES_CREATE_TIMEOUT"] += GetDefaultInt(extraImageDict.ImageCreationTimeout) + ","
		...
		envVars["TEMPEST_EXTRA_IMAGES_FLAVOR_RAM"] += GetDefaultInt(extraImageDict.Flavor.RAM, "-") + ","
		envVars["TEMPEST_EXTRA_IMAGES_FLAVOR_DISK"] += GetDefaultInt(extraImageDict.Flavor.Disk, "-") + ","
		envVars["TEMPEST_EXTRA_IMAGES_FLAVOR_VCPUS"] += GetDefaultInt(extraImageDict.Flavor.Vcpus, "-") + ","
	}
