...
import (
	...
	"github.com/openstack-k8s-operators/test-operator/internal/controller"
...
func main() {
	...
	tempestReconciler := &controller.TempestReconciler{}
	...
	tobikoReconciler := &controller.TobikoReconciler{}
	...
	horizonTestReconciler := &controller.HorizonTestReconciler{}
	...
	ansibleTestReconciler := &controller.AnsibleTestReconciler{}
	ansibleTestReconciler.Client = mgr.GetClient()
	ansibleTestReconciler.Scheme = mgr.GetScheme()
	ansibleTestReconciler.Kclient = kclient
	if err := ansibleTestReconciler.SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "AnsibleTest")
		os.Exit(1)
	}

	testv1beta1.SetupDefaults()

	checker := healthz.Ping

	// nolint:goconst
	if os.Getenv("ENABLE_WEBHOOKS") != "false" {
		if err := webhookv1beta1.SetupTempestWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "Tempest")
			os.Exit(1)
		}
		if err := webhookv1beta1.SetupTobikoWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "Tobiko")
			os.Exit(1)
		}
		if err := webhookv1beta1.SetupHorizonTestWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "HorizonTest")
			os.Exit(1)
		}
		if err := webhookv1beta1.SetupAnsibleTestWebhookWithManager(mgr); err != nil {
